...
function compile(programCode){
        syntaxAnalizator = new SA(states);
        createLexemTable(programCode);
        createConsTable();
      ...
      //  try {
          syntaxAnalizator.program();
          drawer.drawSyntaxTable(syntaxTable);
      //  }
      //  catch(err) {
        //  if (syntaxAnalizator.iterator >= lexemes.length) 
         // console.log("missing end of program");
        //}
        //console.log(syntaxTable);
        setParametresDefault();
}

 function setParametresDefault(){
   lexemNumber = 1;
   idnCode = 1;
   conCode = 1;
   text = "";
   lastIDNType = undefined;
   lexemCode = 0;
   lexema = "";
   state = 1;
   i = 0;
   line = 1;
   lexemes.splice(0, lexemes.length);
   idns.splice(0, idns.length);
