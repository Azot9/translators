function Draw(){
	...
	this.drawPrefixGrammarTable = function(grammar) {
		let tbodyElem = document.getElementById("tbodyElements"); 
		let line, column, row;
			...
			line.style.scope="col";
			tbodyElem.appendChild(row);

		for (rules in grammar) {
			line = document.createElement("th");
			line.appendChild(document.createTextNode(rules));
			row.appendChild(line);
			line.style.scope="col";
			tbodyElem.appendChild(row);
		}
		for (rulesfirstRow in grammar) {

			row = document.createElement("tr");
			line = document.createElement("td");
			line.appendChild(document.createTextNode(rulesfirstRow));
			row.appendChild(line);
			line.style.scope="col";
			tbodyElem.appendChild(row);
			...
			for (rules in grammar) {
				line = document.createElement("td");
				if (
					grammar[rulesfirstRow].equels !== undefined || 
					grammar[rulesfirstRow].less !== undefined ||
					grammar[rulesfirstRow].more !== undefined
				) {
					if (grammar[rulesfirstRow].equels !== undefined) {
						for (let i = 0; i < grammar[rulesfirstRow].equels.length; i++) {
							if (grammar[rulesfirstRow].equels[i] === rules) {
								line.appendChild(document.createTextNode("="));
							}
						}
					}
					if (grammar[rulesfirstRow].less !== undefined) {
						for (let i = 0; i < grammar[rulesfirstRow].less.length; i++) {
							if (grammar[rulesfirstRow].less[i] === rules) {
								line.appendChild(document.createTextNode("<"));
							}
						}
					}
					if (grammar[rulesfirstRow].more !== undefined) {
						for (let i = 0; i < grammar[rulesfirstRow].more.length; i++) {
							if (grammar[rulesfirstRow].more[i] === rules) {
								line.appendChild(document.createTextNode(">"));
							}
						}
					}
